...
- 管制仕様（上位契約）: [ANTIGRAVITY_AGENT_CONTROL_SPEC.md](../ANTIGRAVITY_AGENT_CONTROL_SPEC.md)
...
- **呼び出し順（確定）**
  - 必須: `create-repo-from-folder` → （必要なら）`build-app-simple` → （必要なら）`branding-intake` → `update-convoy-identity` → `review-repo-quality`

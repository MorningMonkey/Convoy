---
description: リポジトリを「Convoy」ブランド に更新し、READMEレイアウトを整備します
---
# 🌸 Convoy Identity Update

このワークフローは、リポジトリのIdentityを「Convoy」スタイル に統一し、READMEのレイアウトを標準化します。
...
- `README.md` を編集して以下の構成にします：
  ...
  4. **バッジ**: 既存のバッジを維持。
  5. **Convoy Note**: 以下のAlertを概要の直後または末尾に追加します。
     ```markdown
     > [!NOTE]
     > このリポジトリはConvoyで作成されました
     > https://github.com/MorningMonkey/Convoy
     ```
